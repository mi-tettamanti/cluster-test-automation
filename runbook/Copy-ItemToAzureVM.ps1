...
workflow Copy-ItemToAzureVM {
    ...
    InlineScript {
        ...
        Write-Verbose ("Retrieving local content from $Using:LocalPath")
        
		# Get the file contents locally
		$reader = New-Object System.IO.BinaryReader([System.IO.File]::OpenRead($Using:LocalPath))

        DO
		{
            $Content = $reader.ReadBytes($Using:BufferSize)
            ...
            if ($Content.Length -gt 0)
			{
		        Invoke-Command -ScriptBlock {
					...
					$Content | Add-Content –Path $RemotePath -Encoding Byte
